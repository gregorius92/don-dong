<x-app-layout>
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="font-display font-black text-2xl text-slate-900 leading-tight">
                    Edit Toko: <span class="text-emerald-700">{{ $store->name }}</span>
                </h1>
                <p class="text-xs text-slate-500 font-medium mt-0.5">
                    Perbarui informasi alamat, nomor WhatsApp, jam operasional, dan titik koordinat Google Maps
                </p>
            </div>
    ...
    <div class="py-8 sm:py-10"
         x-data="{
             storeName: '{{ addslashes(old('name', $store->name)) }}',
             storeCity: '{{ addslashes(old('city', $store->city)) }}',
             storeAddress: '{{ addslashes(old('address', $store->address)) }}',
             storeLat: '{{ old('latitude', $store->latitude) }}',
             storeLng: '{{ old('longitude', $store->longitude) }}',
             embedCode: '{{ addslashes(old('maps_embed', $store->maps_embed ?? '')) }}',
             zoomLevel: 16,
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <form action="{{ route('admin.stores.update', $store) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                        ...
                        <div class="bg-white rounded-3xl border border-slate-200 shadow-xs p-6 sm:p-7 space-y-5">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text"
                                           name="opening_hours"
                                           value="{{ old('opening_hours', $store->opening_hours) }}"
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <textarea name="address"
                                              ...
                                              class="w-full text-sm rounded-xl bg-white text-slate-900 font-medium placeholder-slate-400 border-slate-300 shadow-xs focus:border-emerald-600 focus:ring-emerald-600 leading-relaxed">{{ old('address', $store->address) }}</textarea>
                                    @error('address')
                                        <p class="text-xs font-bold text-red-600 mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <input type="text"
                                           name="maps_embed"
                                           x-model="embedCode"
                                           value="{{ old('maps_embed', $store->maps_embed) }}"
